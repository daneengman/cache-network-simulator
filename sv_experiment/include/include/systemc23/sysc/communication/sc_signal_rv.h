...
extern "C" {
typedef int (*T_sc_snps_hsio)(int, const char *, const char *, const char);
  extern T_sc_snps_hsio C_sc_snps_hsio;
}
...
sc_signal_rv<W>::write( const data_type& value_ )
{
    ...
    bool found = false;
    int mode = 0;  // 0: default  1: write sc->v  2: write v->sc  other: illegal

    if (C_sc_snps_hsio) {
      mode = C_sc_snps_hsio(0, this->name(), "", (char)0);
    }
    ...
    if (mode == 1) {
      data_type new_val;
      sc_lv_resolve<W>::resolve(new_val, this->m_val_vec);
      C_sc_snps_hsio(1, this->name(), new_val.to_string().c_str(), (char)0);
    }
...
sc_signal_rv<W>::update()
{
  int mode = 0;
  if (C_sc_snps_hsio) {
    mode = C_sc_snps_hsio(0, this->name(), "", 0);
  }
  if (mode == 0) {
