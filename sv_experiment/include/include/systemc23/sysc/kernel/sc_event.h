...
private:
    ...
    sc_event_timed( const sc_time& delta, sc_event* e )
	: m_event( e ), m_notify_time( e->m_simc->time_stamp() )
	{ m_notify_time += delta; }
...
sc_event::notify_internal( const sc_time& t )
{
    if( t == SC_ZERO_TIME ) {
        ...
        m_notify_type = DELTA;
        //	sc_core::sc_snps_set_pending_activity(true);
    } else {
        sc_event_timed* et =
        ...
        m_timed = et;
        m_notify_type = TIMED;
        //	sc_core::sc_snps_set_pending_activity_timed(true, t);
    }
}
