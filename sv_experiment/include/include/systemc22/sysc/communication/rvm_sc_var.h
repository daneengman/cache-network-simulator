...
private:
    // Hint for SC_REPORT_ERROR(151 ...)  and similar:
    // The feature is proprietary to VCS-SystemC kernel; using
    // rvm_sc_var with SystemC override flow is not supported.
    // We use the same libbfSim/libbfCbug for VCS-SystemC and override
    // and these libs contain support for rvm_sc_sig, so there is
    // a potential cross-dependency to VCS-SystemC kernel. 
    // Avoid this by not using SC_ID_RVM_SC_VAR_NI_ or anything
    // else (1) defined in the SC kernel and (2) prop. to VCS-SystemC.
    // Instead use integrer literals which makes this class header
    // self-contained. 
    
    // helper methods
    void tni() const {
	SC_REPORT_ERROR(151 /*SC_ID_RVM_SC_VAR_NI_*/,
			"rvm_sc_var<T>::trace(.)");
    }
    void wni() const {
	SC_REPORT_ERROR(151 /*SC_ID_RVM_SC_VAR_NI_*/,
...
rvm_sc_var<T>::rvm_sc_var( const char* name, T* var_p)
    : sc_prim_channel( name ), m_var_p( var_p ), m_delta( ~sc_dt::UINT64_ONE )
{ 
...
void rvm_sc_var<T>::set_var(T* var_p)
{
	...
	SC_REPORT_ERROR(150 /*SC_ID_RVM_SC_VAR_USAGE_*/, "set_var(T* var) used with NULL pointer");
    }
}  
