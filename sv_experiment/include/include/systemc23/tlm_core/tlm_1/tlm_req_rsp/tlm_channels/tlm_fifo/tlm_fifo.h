...
public:
    ...
    static  void sc_snps_print_helper(::std::ostream& s, T* buf, size_t ix);
...
private:
    void sc_snps_prep_for_debug();   // SNPS added

public:
    const sc_core::sc_event& data_read_event() const   // SNPS added
	{ return m_data_read_event; }
    const sc_core::sc_event& data_written_event() const   // SNPS added
	{ return m_data_written_event; }
...
tlm_fifo<T>::init( int size_ ) {
  ...
  sc_snps_prep_for_debug();
}

template <class T>
inline
void
tlm_fifo<T>::sc_snps_prep_for_debug()
{
#ifdef VCS_SYSTEMC
    if (C_sc_snps_sc_fifo_register_print_helper)
        C_sc_snps_sc_fifo_register_print_helper((sc_object*)this, (void*)tlm_fifo<T>::sc_snps_print_helper);
#endif
}
...
#ifdef VCS_SYSTEMC
    if (C_sc_snps_sc_fifo_update)
        C_sc_snps_sc_fifo_update(this, sc_snps::UPDATE);
#endif
}
...
inline
void
tlm_fifo<T>::print( ::std::ostream& os ) const
{
    if( buffer.m_free != m_size ) {
        int i = buffer.m_ri;
        do {
            const T& elem = reinterpret_cast<T*>(buffer.m_buf)[i];
            os << elem << ::std::endl;
            i = ( i + 1 ) % buffer.size();
        } while( i != buffer.m_wi );
    }
...
inline
void
tlm_fifo<T>::dump( ::std::ostream& os ) const
{
...
template <class T>
void
tlm_fifo<T>::sc_snps_print_helper(::std::ostream& s, T* buf, size_t ix)
{
    s << buf[ix];
}
