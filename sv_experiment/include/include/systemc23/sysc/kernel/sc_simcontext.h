...
//FIXME_merg23: cleanup   bool sc_pending_activity_at_current_time();
sc_dt::uint64 sc_snps_get_delta_count();
void sc_snps_reset_delta_count();
void sc_snps_set_pending_activity(bool);
void sc_snps_set_pending_activity_timed(bool, const sc_time &);
...
extern void sc_start( double duration );  /*snps-added*/
extern void sc_stop();
...
// all OK, no systemc pending events, but is still runnable
#define SNPS_CRUNCH_STATUS_OK 0
// error occured during method evalation
#define SNPS_CRUNCH_STATUS_ERROR 1
// stop condition met
#define SNPS_CRUNCH_STATUS_STOP 2
// no more pending events, still runable
#define SNPS_CRUNCH_STATUS_EMPTY 3
// somebody hit the pause button
#define SNPS_CRUNCH_STATUS_PAUSE 4
...
public:
    ...
    bool get_paused() const;  /* snps-added */
    ...
    bool sc_pending_activity_at_current_time() const; //FIXME_merg23: make BF use variant above and rm this one

    /*snps-added-begin*/
    const sc_time next_time() const
        { sc_time t; (void) next_time(t); return t; }
...
private:
    ...
    void crunch_inlined( bool once=false );
 public:
    int crunch_1d();
    int crunch_1dc();
    int crunch_1du();
...
private:
    ...
    void pop_timed_event(); // can assert
    inline sc_event_timed * peek_timed_event() const;
    inline void trigger_timed_events();
    void dump_timed_events();

    inline sc_time simulate_init(const sc_time &);
    inline bool simulate_current_events_and_trace();
    inline bool something_to_trace();
    inline sc_time next_slot_or_UTE( const sc_time& until_t,
				     const sc_time& next_slot );
    inline bool nothing_to_be_done();

public: // for babelfish

    void           advance_simulation(const sc_time &until_t);
    void           advance_simulation_inlined(const sc_time &until_t);
    int            check_simulation_is_stopped();
    int            check_simulation_is_stopped_inlined();
    unsigned       drop_cancelled_events();
    const sc_time& peek_timed_event_notify_time() const;
    void           update_traces();
    void           update_traces_delta(bool delta);
    void           update_traces_inlined();
    void           simulate_current_events();
    int            timed_events_cnt() const;
    bool           sc_snps_pending_activity();

    void set_current_time(const sc_time &ut) { m_curr_time = ut; }
...
inline
bool
sc_simcontext::get_paused() const
{
    return m_paused;
}
/* END snps-added */
