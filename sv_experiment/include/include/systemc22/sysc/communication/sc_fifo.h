...
public:
    ...
    {
        init( size_ );
        sc_snps_prep_for_debug();   // SNPS added
    }
			    ...
			    const char* if_typename_ )
{
    ...
    // FIXME: typeid() can cause memory leak on Sun Compiler
...
sc_fifo<T>::read( T& val_ )
{
    while( num_available() == 0 ) {
        if (C_sc_snps_sc_fifo_update)
            C_sc_snps_sc_fifo_update(this, sc_snps::STUCK_IN_READ);
...
sc_fifo<T>::write( const T& val_ )
{
    while( num_free() == 0 ) {
        if (C_sc_snps_sc_fifo_update)
            C_sc_snps_sc_fifo_update(this, sc_snps::STUCK_IN_WRITE);
...
template <class T>
void
sc_fifo<T>::sc_snps_print_helper(::std::ostream& s, T* buf, size_t ix)
{
    s << buf[ix];
}
...
inline
void
sc_fifo<T>::sc_snps_prep_for_debug()
{
    if (C_sc_snps_sc_fifo_register_print_helper)
        C_sc_snps_sc_fifo_register_print_helper((sc_object*)this, (void*)sc_fifo<T>::sc_snps_print_helper);
}
