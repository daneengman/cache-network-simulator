...
/* VCS-SYSTEMC BEGIN added new message types */
SC_DEFINE_MESSAGE(SC_ID_NOPROC_WAIT_NOT_ALLOWED_ , 596,
	"calling SystemC wait() is not allowed in DPI or PLI functions" )
SC_DEFINE_MESSAGE(SC_ID_STACK_MEMORY_LEAK_  , 597,
        "Stack memory could not be freed. This memory leak usually does not influence the simulation results. Please inform Synopsys Technical Support about this internal problem." )
SC_DEFINE_MESSAGE(SC_ID_ASYNC_RESET_SIGNAL_IS_NOT_ALLOWED_ , 598,
	"async_reset_signal_is() is only allowed for SC_CTHREADs" )
SC_DEFINE_MESSAGE(SC_ID_USE_PTHREADS_, 599,
	"cannot select pthreads" )
/* VCS-SYSTEMC END */
